/**
 * The keymap is data, so the help overlay and the key handler can never drift
 * apart: `bindings()` is the single source of truth for both.
 */
...
export function globalBindings(): Binding[] {
  return [
    ...
    },
    {
      ...
      run: () => setState({ focus: "side", filter: "" }),
    },
    {
      keys: ["r"],
      label: "r",
      description: "refresh this panel",
      hint: true,
      run: () => void refresh.current(),
    },
    {
      keys: ["ctrl+r"],
      label: "C-r",
      description: "refresh everything",
      run: () => void refresh.all(),
    },
    ...
    },
    {
      ...
      run: () =>
        openOverlay({
          ...
          body: "Match against name or path. Empty clears the filter.",
          initial: state.filter,
          onSubmit: (value) => setState({ filter: value.trim(), cursor: { ...state.cursor, repos: 0 } }),
        }),
    },
    {
      keys: ["y"],
